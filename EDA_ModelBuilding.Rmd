...
polyLogModel <- nnet::multinom(ADCDRSTG ~ poly(Del_word_list_memory, 2) + poly(MMSE_score, 2) + 
                         poly(Memory, 2) + poly(Organization, 2) + poly(Boston_naming_test) + poly(Arithmetic),
                         data = train_data)
...
```{r}
# Polynomial Logistic Regression
fit2 <- nnet::multinom(y ~ poly(PC1, 3) + poly(PC2, 2) + poly(PC3, 2) + poly(PC4, 2) +  poly(PC5, 2) + poly(PC6, 2) + 
                                 poly(PC7, 2) + poly(PC8, 2) +  poly(PC9, 2) + poly(PC10, 2) + poly(PC11, 2) + poly(PC12, 2) + 
                                 poly(PC13, 2) + poly(PC14, 2),
                         data = pcaData)
...
ridge_mod = glmnet(x, y, alpha = 0, lambda = seq)

cv <- cv.glmnet(x, y, family = "multinomial", alpha = 0, lambda = lambda_seq)
bestlam = cv$lambda.min

fit <- glmnet(x, y, family = "multinomial", alpha = 0, lambda = bestlam)

# Prediction on test set
#predict_ridge = predict(fit, newdata = test_data)
#confusion= table(Predicted = predict_ridge,
#                      Actual = test_data$ADCDRSTG)
#confusion
```
