...
```{r}
# Optimized model
optRf = randomForest(ADCDRSTG~., data = train_data, mtry = 15)
...
table(Predicted = optPreds, Actual = test_data$ADCDRSTG)

print(paste("The accuracy is", round(opt_rf_acc,2)))

misclass = 1-opt_rf_acc
print(paste("The misclassification error rate is", round(misclass,2)))
```
...
n <- nrow(train_data)
k <- length(ldaMod$svd)
logLik_val <- sum(log(predict(ldaMod, train_data)$x + 4))
...
#KNN
```{r}
train_data_scaled[-c(38)] = scale(cardio_scaled[c(1,3,4,5,6)])
...
```
```{r}
# Calculate F-score for polynomial logistic regression

# precision = PPV
# recall = TPR/ sensitivity

f_score_1 <- 2 * (0.7857 * 0.7586) / (0.7857 + 0.7586)
f_score_2 = 2*(0.7692*0.8140)/(0.7692+0.8140)
f_score_3 = 2*(0.4*0.4)/(0.4+0.4)
f_score_4 = 2*(0.57143*0.44444)/(0.57143+0.44444)

averagef = (f_score_1+f_score_2+f_score_3+f_score_4)/4
```
...
# Calculate accuracy
tab = table(optRadPred,scaled_test$ADCDRSTG)
tab
...
lda_aic <- -2*logLik(ldaMod) + 2*(length(ldaMod$prior) + length(ldaMod$counts))
```

```{r}
# SVM
svm_aic = AIC(optRadSVM)
