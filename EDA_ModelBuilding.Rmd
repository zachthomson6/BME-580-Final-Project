...
polyLogModel_pca <- nnet::multinom(y ~ poly(PC1, 3) + poly(PC2, 2) + poly(PC3, 2) + poly(PC4, 2) +  poly(PC5, 2) + poly(PC6, 2) + 
...
```{r}
# Polynomial Logistic Model with original variables
...
fit1 <- nnet::multinom(formula = ADCDRSTG~., data = train_data)
...
```{r}
# Using Ridge to select variables
x <-  model.matrix(ADCDRSTG ~., data=train_data)[,-1]
y <- train_data[,"ADCDRSTG"]

# Fit Ridge to logistic regression
lambda_seq <- 10^seq(-4, 2, by = 0.1)

cv <- cv.glmnet(x, y, family = "multinomial", alpha = 0, lambda = lambda_seq)
