...
#Logistic Model
```{r}
#logMod = glm(ADCDRSTG~., family = binomial, data = train_data)
#summary(logMod)

#logMod = predict(logMod, newdata = test_data, type = 'response')

# Multinomial Logistic Regression (https://www.r-bloggers.com/2020/05/multinomial-logistic-regression-with-r/)
multinom_model <- multinom(ADCDRSTG ~ ., data = train_data)
...
```
```{r}
# Generate ROC curve for multinomial logistic regression
roc_prob <- data.frame(probs = predict(multinom_model, newdata = train_data, type = "prob"))
predict_roc = prediction(roc_probs$probs, train_data$ADCDRSTG)
...
print(paste("The accuracy of multinomial logistic regression is", acc_LDA))
...
polyLogModel <- multinom(Species ~ poly(Sepal.Length, 2) + poly(Sepal.Width, 2) + 
                         poly(Petal.Length, 2) + poly(Petal.Width, 2), 
                         data = trainData)

# Make predictions on test data
predicted <- predict(polyLogModel, testData)
