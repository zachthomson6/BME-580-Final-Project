...
multinom_model <- nnet::multinom(formula = ADCDRSTG~., data = train_data)
...
print(paste("The accuracy of multinomial logistic regression is", acc_multimod))
```
```{r}
# Generate performance stats for multinomial logistic regression
roc_prob <- data.frame(probs = predict(multinom_model, newdata = test_data, type = "prob"))
predict_multinom = colnames(roc_prob)[apply(roc_prob, 1, which.max)]

# Overall performance stats of classification decision tree model
predict_multinom = factor(predict_multinom,levels = c(0,0.5,1,2,3,4,5))
tab_multinom <- confusionMatrix(predict_multinom, test_data$ADCDRSTG)
tab_multinom

#predict_roc = prediction(roc_prob$probs, train_data$ADCDRSTG)
#performance_roc = performance(predict)

#roc <- roc(train_data$ADCDRSTG, roc_prob$prob)
#plot(roc, main = "ROC Curve for Multinomial Logistic Regression")
```
...
# Calculating accuracy - sum of diagonal elements divided by total obs
acc_LDA = round((sum(diag(confusionTab_LDA_test))/sum(confusionTab_LDA_test))*100,2)
print(paste("The accuracy of LDA is", acc_LDA))
...
polyLogModel <- nnet::multinom(ADCDRSTG ~ poly(Del_word_list_memory, 2) + poly(MMSE_score, 2) + 
                         poly(Memory, 2) + poly(Organization, 2), 
                         data = train_data)
...
```{r}
pwMod <- segmented(glm(ADCDRSTG~., data = train_data))

# Make predictions on test data
probabilities <- predict(pw_model, newdata = test_data, type = "response")
```

#GAMS (error)
```{r}
#gamMod = gam(ADCDRSTG ~ s(Del_word_list_memory) + s(MMSE_score) + s(Memory) +
 #              s(Organization) + s(Judgement) + s(Construction_praxis_score),
  #           data = train_data, family = multinom(K=7))

gamMod = gam(ADCDRSTG ~ Del_word_list_memory + MMSE_score + Memory +
               Organization + Judgement + Construction_praxis_score,
             data = train_data, family = multinom(K=6))
...
# Compute accuracy
accuracy <- mean(predictions == test_data$ADCDRSTG)
print(paste("Accuracy:", accuracy))
```
